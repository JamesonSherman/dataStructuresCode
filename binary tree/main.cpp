...
const char DESCRIPTION[MANY_TESTS+1][256] = {
	...
	"Testing start, is_current, get_current, and advance functions"
};
...
// **************************************************************************
// int test5( )
//   Performs basic tests for the internal iterator functions.
...
int test5()
{
	...
	cout << test_letter++ << ". ";
	cout << "Adding the number 4 to the bag, and then testing\n";
	cout << "   start, is_current, get_current, and advance.";
	cout << endl;
	test.insert(4);
	items[4]++;
	if (!correct(test, 1, items, 7)) return 0;
	...
	if (!test.is_current())
	{
		cout << "   is_current returned false. Test failed." << endl;
		return 0;
	}
	if (test.get_current() != 4)
	{
		cout << "   get_current returned an incorrect value. Test failed." << endl;
		return 0;
	}
	test.advance();
	if (test.is_current()) 
	{
		cout << "   is_current returned true. Test failed." << endl;
		return 0;
	}

	cout << test_letter++ << ". ";
	cout << "Inserting 2, 1, 3, 2, 5, 6, 7 into the bag.\n";
	cout << "   Then testing internal iterator functions.";
	cout << endl;
	test.insert(2);
	items[2]++;
	...
	if (!test.is_current())
	{
		cout << "   checking is_current. Test failed." << endl;
		return 0;
	}
	if (test.get_current() != 7)
	{
		cout << "   checking get_current. Test failed." << endl;
		return 0;
	}
	test.advance(); 
	test.advance(); 
	...
	if (test.get_current() != 4)
	{
		cout << "   calling advance three times and checking get_current. Test failed." << endl;
		return 0;
	}
	test.advance();
	test.advance();
	if (test.get_current() != 2)
	{
		cout << "   calling advance two more times and checking get_current. Test failed." << endl;
		return 0;
	}
	test.advance();
	test.advance(); 
	...
	if (test.is_current())
	{
		cout << "   calling advance three more times and checking is_current. Test failed." << endl;
		return 0;
	}
	...
	if (test.get_current() != 7)
	{
		cout << "   calling start again and get_current. Test failed." << endl;
		return 0;
	}
	...
	if (test.is_current())
	{
		cout << "   calling insert and checking is_current. Test failed." << endl;
		return 0;
	}
	...
	{
		cout << "   Removing 0 (which is not in bag, so bag should be unchanged) ... ";
		cout << "   and checking is_current. Test failed." << endl;
		return 0;
	}
	...
	if (test.is_current())
	{
		cout << "   calling erase(7) and checking is_current. Test failed." << endl;
		return 0;
	}

	test.start();
	if (test.get_current() != 6)
	{
		cout << "   calling start again and get_current. Test failed." << endl;
		return 0;
	}
	...
	if (!test.is_current())
	{
		cout << "   Removing 7 (which is not in bag, so bag should be unchanged) ... ";
		cout << "   and checking is_current. Test failed." << endl;
		return 0;
	}
	...
	if (test.is_current())
	{
		cout << "   calling erase_one(1) and checking is_current. Test failed." << endl;
		return 0;
	}
	cout << "   Testing size and count.  ";
	if (!correct(test, 6, items, 7)) return 0;
	test.start();
	cout << "   Calling start and testing size and count.  ";
	if (!correct(test, 6, items, 7)) return 0;
	test.advance();
	cout << "   Calling advance and testing size and count.  ";
	if (!correct(test, 6, items, 7)) return 0;
	return POINTS[5];
}
...
int main( )
{
    ...
    cout << "You have " << sum << " points out of the ";
    cout << POINTS[0];
