...
// [https://www.cs.colorado.edu/~main/chapter6/sequence_exam4.cxx]:
...
#include "sequence5.h"  // Provides the template sequence class
using std::cout;
using std::endl;
...
const unsigned int POINTS[MANY_TESTS+1] = {
...
const char DESCRIPTION[MANY_TESTS+1][256] = {
    "tests for sequence class implemented with two stacks",
...
// **************************************************************************
// bool test_basic(const sequence<unsigned int>& test, size_t s, bool has_cursor)
...
// **************************************************************************
// bool test_items(sequence<unsigned int>& test, size_t s, size_t i, unsigned int items[])
...
// **************************************************************************
// bool correct
//   (sequence<unsigned int>& test, size_t s, size_t cursor_spot, unsigned int items[])
...
// **************************************************************************
bool correct
(sequence<unsigned int>& test, size_t size, size_t cursor_spot, unsigned int items[])
{
...
unsigned int test1( )
{
    sequence<unsigned int> empty;                // An empty sequence
    sequence<unsigned int> test;                 // A sequence to add items to
    unsigned int items1[4] = { 5, 10, 20, 30 };  // These 4 items are put in a sequence
    unsigned int items2[4] = { 10, 15, 20, 30 }; // These are put in another sequence
...
unsigned int test2( )
{
	...
	// Attach 100 at the end and test retreat
	cout << "Attaching 100, which should go at the sequence's end." << endl;
	cout << "Then calling retreat four times to run cursor off the sequence ...";
	test.attach(100);
...
unsigned int test3( )
{
    ...
    unsigned int items1[1] = { 30 };
    unsigned int items2[2] = { 10, 30 };
    unsigned int items3[3] = { 10, 20, 30 };
...
{
    const size_t TESTSIZE = 30;
    sequence<unsigned int> original; // A sequence that we'll copy.
    unsigned int items[2*TESTSIZE];
    size_t i;
    ...
    sequence<unsigned int> copy1(original);
    ...
    for (i=2; i <= 2*TESTSIZE; i++)
        original.attach(i);
    sequence<unsigned int> copy2(original);
    ...
    sequence<unsigned int> copy3(original);
    original.start( );
    original.advance( );
    original.remove_current( ); // Delete 2 from original, but not the copy.
    if (!correct
    ...
    sequence<unsigned int> copy4(original);
    original.start( );
    original.advance( );
    original.remove_current( ); // Delete 2 from original, but not the copy.
    if (!correct
    ...
    sequence<unsigned int> copy5(original);
    original.start( );
    original.advance( );
    original.remove_current( ); // Delete 2 from original, but not the copy.
    if (!correct
...
{
    const size_t TESTSIZE = 30;
    sequence<unsigned int> testa, testi;
    unsigned int items[2*TESTSIZE];
    size_t i;
